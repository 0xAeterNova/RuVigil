 ...
 */
export async function runCog(binary, args) {
    return new Promise((resolve) => {
        ...
        child.on("error", (e) => {
            resolve(err(`Failed to launch cog binary "${binary}" (${args.join(" ")}): ${e.message}. ` +
        ...
        child.on("close", (code) => {
            if (code !== 0) {
                resolve(err(`Cog "${binary} ${args.join(" ")}" exited with code ${code}. ` +
 ...
 */
export async function cogHealth(binary) {
    return runCog(binary, ["health"]);
}
/**
 * Call `cog-<id> version` and return the version string.
 */
export async function cogVersion(binary) {
 ...
 */
export async function cogInferStub(binary, taskLabel) {
    const health = await cogHealth(binary);
    if (!health.ok) {
        return err(`[WARN] ${taskLabel} cog health check failed — ${health.error}. ` +
