...
export const csiLatestSchema = z.object({
    ...
    sensing_server_url: z
        ...
        .url()
        .optional()
        .describe("Base URL of the sensing-server (default: RUVIEW_SENSING_SERVER_URL or http://localhost:3000)"),
});
export async function csiLatest(input, config) {
    const baseUrl = input.sensing_server_url ?? config.sensingServerUrl;
    const result = await sensingGet(baseUrl, "/api/v1/sensing/latest", config.apiToken);
    if (!result.ok) {
        return {
            ok: false,
            warn: true,
            error: result.error,
            hint: "Ensure the wifi-densepose-sensing-server is running. " +
    ...
    if (!validation.valid) {
        return {
            ...
            hint: "The sensing-server may have upgraded its schema. " +
...
//# sourceMappingURL=csi-latest.js.map