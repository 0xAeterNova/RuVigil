...
export const trainCountSchema = z.object({
    ...
    output_dir: z
        .string()
        .optional()
        .describe("Directory for model artifacts (default: v2/crates/cog-person-count/cog/artifacts/)."),
});
...
export async function trainCount(input, config) {
    ...
    const outputDir = input.output_dir ?? "v2/crates/cog-person-count/cog/artifacts";
    ...
    return {
        ok: true,
        result,
        note: "Training job spawned in the background. " +
            `Poll progress with ruview_job_status({ job_id: "${jobId}" }). ` +
            `Live log: ${logPath}`,
    };
}
export async function jobStatus(input, _config) {
...
//# sourceMappingURL=train-count.js.map