...
export async function bfldSubscribe(input, config) {
    ...
    const result = await sensingGet(baseUrl, `/api/v1/bfld/${encodeURIComponent(nodeId)}/subscribe?duration_s=${input.duration_s}`, config.apiToken);
    if (!result.ok) {
        ...
        return {
            ...
            topic,
            error: result.error,
            hint: "Sensing-server not reachable — subscription envelope is synthetic. " +
...
//# sourceMappingURL=bfld-subscribe.js.map