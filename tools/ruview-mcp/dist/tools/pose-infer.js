...
export async function poseInfer(input, config) {
    ...
    if (!healthResult.ok) {
        return {
            ...
            hint: "Set RUVIEW_POSE_COG_BINARY to the path of the cog-pose-estimation binary. " +
    ...
    return {
        ok: true,
        synthetic_window: true,
        note: "M2: inference ran on a synthetic CSI window via `cog-pose-estimation health`. " +
            "For real CSI window inference, provide window_path (M3) or ensure the sensing-server is running.",
        result,
    };
}
//# sourceMappingURL=pose-infer.js.map