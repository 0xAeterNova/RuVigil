...
function parseCountHealthOutput(stdout) {
...
export async function countInfer(input, config) {
    ...
    if (!healthResult.ok) {
        return {
            ...
            hint: "Set RUVIEW_COUNT_COG_BINARY to the path of the cog-person-count binary. " +
    ...
    if (!healthEvent) {
        ...
        return {
            ...
            note: "Cog health passed (exit 0) but no health.ok event was parseable. " +
    ...
    return {
        ok: true,
        synthetic_window: true,
        note: "M2: inference ran on a synthetic CSI window via `cog-person-count health`. " +
            "For real CSI window inference, provide window_path (M3) or ensure the sensing-server is running.",
        result,
    };
}
//# sourceMappingURL=count-infer.js.map