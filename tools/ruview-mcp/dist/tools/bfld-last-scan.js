...
export async function bfldLastScan(input, config) {
    const baseUrl = input.sensing_server_url ?? config.sensingServerUrl;
    const nodeId = input.node_id ?? "default";
    const result = await sensingGet(baseUrl, `/api/v1/bfld/${encodeURIComponent(nodeId)}/last_scan`, config.apiToken);
    if (!result.ok) {
        return {
            ok: false,
            warn: true,
            error: result.error,
            hint: "Ensure the sensing-server is running and the BFLD pipeline is active " +
...
//# sourceMappingURL=bfld-last-scan.js.map