...
import { trainCountSchema, trainCount, jobStatusSchema, jobStatus, } from "./tools/train-count.js";
...
const TOOLS = [
    {
        ...
        description: "Pull the latest CSI window from a running wifi-densepose-sensing-server. " +
        ...
        inputSchema: {
            ...
            properties: {
                sensing_server_url: {
                    type: "string",
                    description: "Base URL of the sensing-server (default: RUVIEW_SENSING_SERVER_URL or http://localhost:3000).",
                },
            },
        },
    ...
    },
    {
        ...
        description: "Run a single-shot 17-keypoint COCO pose estimation inference using the " +
    ...
    },
    {
        ...
        description: "Run a single-shot person-count inference using the cog-person-count Cog " +
    ...
    },
    {
        ...
        description: "List cogs from the Cognitum edge module registry (ADR-102). " +
        ...
        inputSchema: {
            ...
            properties: {
                category: {
                    type: "string",
                    description: "Filter by category: health, security, building, retail, industrial, " +
    ...
    },
    {
        ...
        description: "Kick off a cog-person-count training run using the Candle GPU trainer " +
        ...
        inputSchema: {
            ...
            properties: {
                paired_jsonl: {
                    type: "string",
                    description: "Path to the paired JSONL training file (produced by scripts/align-ground-truth.js).",
                },
                epochs: {
                    type: "integer",
                    minimum: 1,
                ...
                output_dir: {
                    type: "string",
                    description: "Directory for model artifacts (default: v2/crates/cog-person-count/cog/artifacts/).",
                },
            },
        },
        handler: async (args, config) => {
    ...
    },
    {
        ...
        description: "Poll the status of a background training job started by ruview_train_count. " +
    ...
    // ── ADR-124 BFLD tools (Phase 4 Refinement) ──────────────────────────────
    {
        ...
        description: "Return the most recent BFLD scan result for a node (ADR-118/ADR-121). " +
        ...
        handler: async (args, config) => {
            return bfldLastScan(args, config);
        },
    },
    {
        name: "ruview.bfld.subscribe",
        description: "Subscribe to BFLD events on ruview/<node_id>/bfld/* for duration_s seconds (ADR-122). " +
        ...
        handler: async (args, config) => {
            return bfldSubscribe(args, config);
        },
    },
    // ── ADR-124 Presence + Vitals tools (Phase 4 Refinement iter 5) ──────────
    {
        name: "ruview.presence.now",
        description: "Return current occupancy for a node: present, n_persons, confidence, timestamp_ms. " +
        ...
        handler: async (args, config) => presenceNow(args, config),
    },
    {
        name: "ruview.vitals.get_breathing",
        description: "Return breathing rate for a node: breathing_rate_bpm (null if unavailable), " +
        ...
        handler: async (args, config) => vitalsGetBreathing(args, config),
    },
    {
        name: "ruview.vitals.get_heart_rate",
        description: "Return heart rate for a node: heartrate_bpm (null if unavailable), " +
        ...
        handler: async (args, config) => vitalsGetHeartRate(args, config),
    },
    {
        name: "ruview.vitals.get_all",
        description: "Return the full EdgeVitalsMessage for a node (all fields except raw): " +
            "presence, n_persons, confidence, breathing_rate_bpm, heartrate_bpm, body_temperature_c, motion, zone_id. " +
        ...
        handler: async (args, config) => vitalsGetAll(args, config),
    },
];
// ── Server bootstrap ────────────────────────────────────────────────────────
async function main() {
    ...
    server.setRequestHandler(CallToolRequestSchema, async (request) => {
        ...
        const schemaEntry = Object.prototype.hasOwnProperty.call(TOOL_INPUT_SCHEMAS, name)
            ? TOOL_INPUT_SCHEMAS[name]
            : undefined;
        if (schemaEntry !== undefined) {
            const parsed = schemaEntry.safeParse(args ?? {});
            if (!parsed.success) {
                throw new McpError(ErrorCode.InvalidParams, `Invalid arguments for tool "${name}": ${parsed.error.message}`);
            }
        }
        try {
            ...
            if (e instanceof McpError)
                throw e; // propagate typed errors unchanged
    ...
    process.stderr.write(`[@ruvnet/rvagent] Server v${PACKAGE_VERSION} started. ` +
