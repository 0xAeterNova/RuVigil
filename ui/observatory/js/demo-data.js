/**
 * Demo Data Generator - RuVigil Observatory
 *
 * Generates synthetic CSI data matching the SensingUpdate contract.
 * Generates the configured RuVigil care scenarios.
...
/** Synthetic body temperature channel for demos or fused thermal sensors. */
function _bodyTempC(t, base = 36.7, load = 0, noise = 0) {
  const circadian = Math.sin(t * 0.035) * 0.12;
  return _clamp(base + load + circadian + noise, 34.0, 40.5);
}
...
export class DemoDataGenerator {
  ...
  _fallEvent(t) {
    ...
    return this._baseFrame({
      ...
      vital_signs: {
        ...
        body_temperature_c: _bodyTempC(t, 36.9, postFall ? 0.35 : 0.05, n.env(t) * 0.04),
        body_temperature_confidence: 0.72,
      },
  ...
  _crowdOccupancy(t) {
    ...
    // Person 5: queueing near the counter
    {
      const px = 1.55 + Math.sin(t * 0.21) * 0.28;
      const pz = 1.55 + Math.cos(t * 0.18) * 0.22;
      persons.push({ id: 'p5', position: [px, 0, pz], motion_score: 12 + Math.abs(_harmonicNoise(t, 18.1, 2)) * 8, pose: 'standing', facing: -0.35 });
    }

    // Person 6: slow center movement
    {
      const ct = t % 18;
      if (ct < 14) {
        const px = Math.sin(t * 0.24 + 1.1) * 1.2;
        const pz = Math.cos(t * 0.18 + 0.4) * 0.9;
        persons.push({ id: 'p6', position: [px, 0, pz], motion_score: 55, pose: 'walking', facing: Math.atan2(Math.cos(t * 0.24), -Math.sin(t * 0.18)) });
      }
    }

    // Person 7: intermittent doorway dwell, bringing the max to 8 PPL
    {
      const ct = t % 22;
      if (ct > 5 && ct < 18) {
        const px = -3.1 + Math.sin(t * 0.16) * 0.18;
        const pz = 2.0 + Math.cos(t * 0.12) * 0.16;
        persons.push({ id: 'p7', position: [px, 0, pz], motion_score: 18, pose: 'standing', facing: 0.9 });
      }
    }

    const actualCount = Math.min(persons.length, 8);
    if (persons.length > 8) persons.length = 8;
    ...
    return this._baseFrame({
      ...
      vital_signs: {
        breathing_rate_bpm: 0,
        heart_rate_bpm: 0,
        breathing_confidence: 0.15,
        heart_rate_confidence: 0.1,
        body_temperature_c: 0,
        body_temperature_confidence: 0.15,
      },
      persons,
  ...
  _elderlyCare(t) {
    ...
    return this._baseFrame({
      ...
      vital_signs: {
        ...
        body_temperature_c: _bodyTempC(t, 36.6, walkPhase ? 0.12 : 0.02, n.env(t) * 0.04),
        body_temperature_confidence: 0.76,
  ...
  _fitnessTracking(t) {
    ...
    return this._baseFrame({
      ...
      vital_signs: {
        ...
        body_temperature_c: _bodyTempC(t, 36.8, targetIntensity * 0.65, n.env(t) * 0.05),
        body_temperature_confidence: 0.72,
