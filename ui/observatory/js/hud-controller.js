 ...
 */
import { analyzeFrame } from './ai-advisor.js';

// ---- Constants ----

export const SCENARIO_NAMES = [
  'ELDERLY CARE','FITNESS','FALL DETECT','CROWD OCCUPANCY',
];
...
export const DEFAULTS = {
  ...
  wireColor: '#1de7c3', jointColor: '#ff6b8b', aura: 0.02,
...
export const PRESETS = {
  ...
  minimal: {
    ...
    waves: 0.3, ambient: 0.6, reflect: 0.1, wireColor: '#1de7c3', jointColor: '#5b7cfa',
  },
  neon: {
    ...
    waves: 1.0, ambient: 0.15, reflect: 0.7, wireColor: '#1de7c3', jointColor: '#ff6b8b',
  },
  tactical: {
    ...
    waves: 0.4, ambient: 0.3, reflect: 0.15, wireColor: '#9ff7e6', jointColor: '#f8c846',
  },
  medical: {
    ...
    waves: 0.3, ambient: 0.5, reflect: 0.2, wireColor: '#5b7cfa', jointColor: '#ff6b8b',
  },
};

// Scenario descriptions shown below the dropdown
const SCENARIO_DESCRIPTIONS = {
  auto:              'Auto-cycling through the configured RuVigil care scenarios.',
  fall_event:        'Sudden posture-change detection using acceleration feature analysis.',
  crowd_occupancy:   'Estimating room occupancy count with an eight-person room limit.',
...
const SCENARIO_EDGE_MODULES = {
  auto:              [],
  fall_event:        ['FALL', 'VITALS', 'TEMP'],
  crowd_occupancy:   ['OCCUPANCY'],
  elderly_care:      ['GAIT', 'VITALS', 'FALL', 'TEMP'],
  fitness_tracking:  ['GESTURE', 'GAIT', 'VITALS', 'TEMP'],
};
...
function vitalColor(type, value) {
  ...
  if (type === 'temp') {
    if (value < 35.5 || value >= 39.0) return 'var(--red-alert)';
    if (value < 36.1 || value > 37.8) return 'var(--amber)';
    return 'var(--temp)';
  }
...
export class HudController {
  ...
  updateHUD(data, demoData) {
    ...
    const targetHr = vs.heart_rate_bpm ?? vs.heartrate_bpm ?? vs.hr_proxy_bpm ?? 0;
    const targetBr = vs.respiratory_rate_bpm ?? vs.breathing_rate_bpm ?? vs.breathing_bpm ?? 0;
    const targetTemp = vs.body_temperature_c ?? vs.body_temp_c ?? vs.temperature_c ?? 0;
    ...
    this._lerpTemp = targetTemp > 0 ? lerp(this._lerpTemp || targetTemp, targetTemp, lerpFactor) : 0;
    ...
    this._setWidth('temp-bar', Math.max(0, Math.min(100, ((this._lerpTemp - 34) / 6) * 100)));
    ...
    this._setColor('temp-value', vitalColor('temp', this._lerpTemp));
    ...
    this._setBarColor('temp-bar', vitalColor('temp', this._lerpTemp));
    ...
    this._updateAiAdvisor(analyzeFrame(data));
  }
  ...
  _updateAiAdvisor(advice) {
    if (!advice) return;
    const sevEl = document.getElementById('ai-severity');
    const summaryEl = document.getElementById('ai-summary');
    const recEl = document.getElementById('ai-recommendations');
    if (sevEl) {
      const label = advice.severity === 'critical' ? 'Critical' : advice.severity === 'watch' ? 'Watch' : 'Monitoring';
      sevEl.textContent = label;
      sevEl.className = `ai-severity ai-severity--${advice.severity}`;
    }
    if (summaryEl) summaryEl.textContent = advice.summary || '';
    if (recEl) {
      const items = advice.recommendations || [];
      recEl.innerHTML = items.map(item => `<li>${item}</li>`).join('');
    }
  }
}
