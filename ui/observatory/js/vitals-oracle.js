...
export class VitalsOracle {
  constructor(scene, panelGroup) {
    ...
    // Bloom point light
    this._light = new THREE.PointLight(0x1de7c3, 1.5, 8);
  ...
  update(dt, elapsed, data) {
    ...
    this._breathBpm = vs.respiratory_rate_bpm ?? vs.breathing_rate_bpm ?? vs.breathing_bpm ?? 0;
    this._hrBpm = vs.heart_rate_bpm ?? vs.heartrate_bpm ?? vs.hr_proxy_bpm ?? 0;
    this._breathConf = vs.breathing_confidence || 0;
    this._hrConf = vs.heart_rate_confidence || 0;
    this._bodyTempC = vs.body_temperature_c ?? vs.body_temp_c ?? vs.temperature_c ?? 0;
    this._tempConf = vs.body_temperature_confidence || 0;
    ...
    // Center orb pulse and temperature color
    const orbPulse = 1.0 + breathPulse * 0.1;
    this._orb.scale.set(orbPulse, orbPulse, orbPulse);
    const tempColor = this._bodyTempC >= 38.0 || (this._bodyTempC > 0 && this._bodyTempC < 36.1)
      ? 0xf28f3b
      : 0x1de7c3;
    this._orbMat.color.setHex(tempColor);
    this._light.color.setHex(tempColor);
    this._light.intensity = 0.8 + Math.abs(breathPulse) * 1.0 + this._tempConf * 0.35;
