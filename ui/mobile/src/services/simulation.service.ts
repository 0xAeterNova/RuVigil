...
export function generateSimulatedData(timeMs = Date.now()): SensingFrame {
  ...
  return {
    ...
    vital_signs: {
      ...
      breathing_rate_bpm: breathingBpm,
      hr_proxy_bpm: hrProxy,
      heart_rate_bpm: hrProxy,
      body_temperature_c: isPresent ? 36.7 + motionBand * 1.4 + Math.sin(t * 0.01) * 0.1 : 0,
      body_temperature_confidence: isPresent ? 0.72 : 0,
      confidence,
    },
