...
- **Body temperature** -- carried by the RuVigil host/UI schema when supplied by an external thermometer or fused sensor. The ESP32 CSI firmware does not infer temperature from WiFi CSI alone.
...
- It does **not** run pose estimation. Pose-related features in the host
  ...
  variance, motion-band power), not from learned keypoint inference.
- It does **not** estimate body temperature from CSI. RuVigil temperature
  values must come from an external thermometer, a fused sensor packet, or
  deployment data used by the host-side AI module.
- The repository does not ship pre-trained weights — see issues
...
| `subk_count` | u8 | `16` | Top-K subcarrier count for variance tracking; supports up to 8 RuVigil room slots |
