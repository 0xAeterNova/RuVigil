...
//! - person count     : 1.0 Hz
//! - vitals (HR / BR / temp) : 0.2 Hz (every 5 s)
//! - motion level     : 1.0 Hz
...
fn rate_hz_for(entity: EntityKind, rates: &PublishRates) -> f64 {
    match entity {
        ...
        EntityKind::BreathingRate | EntityKind::HeartRate | EntityKind::BodyTemperature => rates.vitals_hz,
...
#[derive(Serialize, Debug)]
struct TemperatureWithConfidence {
    celsius: f64,
    confidence: f64,
    ts: String,
}
    ...
    pub fn numeric(&self, entity: EntityKind, snap: &VitalsSnapshot) -> Option<StateMessage> {
        ...
        let payload_value: Value = match entity {
            ...
            EntityKind::BodyTemperature => {
                let celsius = snap.body_temperature_c?;
                serde_json::to_value(TemperatureWithConfidence {
                    celsius,
                    confidence: snap.vital_confidence,
                    ts: ts.clone(),
                }).ok()?
...
mod tests {
    ...
    #[test]
    fn numeric_encoder_emits_celsius_payload_for_body_temperature() {
        let b = builder();
        let enc = StateEncoder { builder: &b };
        let s = snap();
        let msg = enc.numeric(EntityKind::BodyTemperature, &s).unwrap();
        let json: serde_json::Value = serde_json::from_str(&msg.payload).unwrap();
        assert_eq!(json["celsius"], 36.8);
        assert_eq!(json["confidence"], 0.87);
    }
    ...
    fn numeric_encoder_returns_none_when_optional_field_missing() {
        ...
        s.body_temperature_c = None;
        assert!(enc.numeric(EntityKind::BodyTemperature, &s).is_none());
    }
