...
impl EntityKind {
    pub fn topic_slug(self) -> &'static str {
        match self {
            ...
            EntityKind::BodyTemperature => "body_temperature",
    ...
    pub fn display_name(self) -> &'static str {
        match self {
            ...
            EntityKind::BodyTemperature => "Body temperature",
...
impl<'a> DiscoveryBuilder<'a> {
    ...
    pub fn build(&self, entity: EntityKind) -> DiscoveryConfig {
        ...
        match entity {
            ...
            EntityKind::BodyTemperature => {
                cfg.state_class = Some("measurement".into());
                cfg.device_class = Some("temperature".into());
                cfg.unit_of_measurement = Some("°C".into());
                cfg.icon = Some("mdi:thermometer".into());
                cfg.value_template = Some("{{ value_json.celsius }}".into());
                cfg.json_attributes_topic = Some(cfg.state_topic.clone());
            }
            EntityKind::MotionLevel => {
                cfg.state_class = Some("measurement".into());
...
mod tests {
    ...
    #[test]
    fn body_temperature_discovery_payload_shape() {
        let b = builder();
        let cfg = b.build(EntityKind::BodyTemperature);
        let j: Value = serde_json::to_value(&cfg).unwrap();
        assert_eq!(j["unit_of_measurement"], "°C");
        assert_eq!(j["device_class"], "temperature");
        assert_eq!(j["state_class"], "measurement");
        assert_eq!(j["value_template"], "{{ value_json.celsius }}");
        assert!(j["json_attributes_topic"].as_str().unwrap().ends_with("/state"));
    }
    ...
    fn enabled_entities_default_excludes_pose_and_includes_all_others() {
        ...
        assert!(entities.contains(&EntityKind::BodyTemperature));
    ...
    fn topic_components_match_entity_kind() {
        ...
        assert_eq!(EntityKind::BodyTemperature.component(), DiscoveryComponent::Sensor);
