...
async fn vital_signs_endpoint(State(state): State<SharedState>) -> Json<serde_json::Value> {
    ...
    Json(serde_json::json!({
        "vital_signs": {
            ...
            "body_temperature_c": vs.body_temperature_c,
...
fn vitals_snapshots_from_sensing_json(
    ...
    let body_temp = vit["body_temperature_c"]
        .as_f64()
        .or_else(|| vit["body_temp_c"].as_f64())
        .or_else(|| vit["temperature_c"].as_f64());
...
mod mqtt_bridge_tests {
    ...
    fn per_node_presence_uses_each_nodes_own_classification() {
        let v = json!({
            ...
            "vital_signs": { "breathing_rate_bpm": 14.0, "heart_rate_bpm": 60.0, "body_temperature_c": 36.8 },
            "persons": [{}, {}],
            "nodes": [
        ...
        assert_eq!(n1.body_temperature_c, Some(36.8));
        assert_eq!(n2.body_temperature_c, Some(36.8));
    ...
    fn falls_back_to_single_aggregate_when_no_nodes() {
        let v = json!({
            ...
            "vital_signs": { "breathing_rate_bpm": 12.0, "body_temp_c": 37.1 },
            "persons": [{}]
        });
        let snaps = vitals_snapshots_from_sensing_json(&v, "ruview");
        assert_eq!(snaps.len(), 1);
        ...
        assert_eq!(snaps[0].body_temperature_c, Some(37.1));
    }
